...
export class MathUtils {
     ...
     */
    static computeEigenDecomposition3x3(matrix) {
    ...
    /**
     * Compute eigenvalues of 3x3 symmetric matrix (simplified version)
     * @deprecated Use computeEigenDecomposition3x3 instead
     */
    static computeEigenvalues3x3(matrix) {
        const m = matrix.elements;
        ...
        // Use roots of characteristic polynomial (simplified method)
        // For symmetric matrices, eigenvalues are real

        // Calculate trace
        const trace = a00 + a11 + a22;

        // Calculate determinant (simplified version)
        // Use simplified Gershgorin circle theorem
        const eigenvalues = [
            Math.abs(a00),
            Math.abs(a11),
            Math.abs(a22)
    ...
    static computeInertiaBox(inertial) {
        const Ixx = inertial.ixx || 0;
        const Iyy = inertial.iyy || 0;
        const Izz = inertial.izz || 0;
        const Ixy = inertial.ixy || 0;
        const Ixz = inertial.ixz || 0;
        const Iyz = inertial.iyz || 0;
        const mass = inertial.mass || 1;

        // Reasonableness check 1: Mass too small (e.g., virtual links in MJCF or placeholders in URDF)
        const minMassThreshold = 0.01; // 10g, values below this may be placeholders
        if (mass < minMassThreshold) {
            // For very small masses, check if inertia radius is reasonable
            const avgInertia = (Math.abs(Ixx) + Math.abs(Iyy) + Math.abs(Izz)) / 3;
            const inertiaRadius = Math.sqrt(avgInertia / mass); // Characteristic inertia radius

            // If inertia radius too large (>0.05m), parameters are unreasonable
            // For very small masses (<10g), reasonable object sizes should be small (e.g., screws, small electronic components)
            // Inertia radius = sqrt(I/m), for 1g object, 5cm inertia radius is already large
            if (inertiaRadius > 0.05) {
                return null; // Don't display inertia box
            }
        }
        ...
        // Check if there are off-diagonal components (URDF may have, MJCF usually doesn't)
        const hasOffDiagonal = Math.abs(Ixy) > 1e-10 || Math.abs(Ixz) > 1e-10 || Math.abs(Iyz) > 1e-10;

        let principalInertias;
        ...
        if (hasOffDiagonal) {
            ...
            // No off-diagonal components, use diagonal values directly (MJCF case)
            principalInertias = [Ixx, Iyy, Izz];
        }

        // For uniformly dense rectangular box, relationship between moment of inertia and dimensions:
        // Ixx = (m/12) * (h^2 + d^2)  (around x-axis)
        // Iyy = (m/12) * (w^2 + d^2)  (around y-axis)
        // Izz = (m/12) * (w^2 + h^2)  (around z-axis)
        // where w=width(x), h=height(y), d=depth(z)

        // Reverse solve to get:
        // w^2 = (6/m) * (Iyy + Izz - Ixx)
        // h^2 = (6/m) * (Ixx + Izz - Iyy)
        // d^2 = (6/m) * (Ixx + Iyy - Izz)
        ...
        // For boxes, inertia radius (I/m)^0.5 should be proportional to box size
        // If inertia radius much larger than box size, parameters are inconsistent
        ...
        // For uniform box, inertia radius should be approximately 0.29 times box size (sqrt(1/12))
        // Allow some error range, but if difference too large, it's unreasonable
        // If inertia radius > average box size, inertia values are too large relative to size
        if (inertiaRadius > avgBoxSize) {
            return null; // Don't display inertia box
        }

        // Reasonableness check 4: Check if equivalent density is reasonable (mass-size ratio check)
        // Calculate equivalent volume and density of box
        const volume = width * height * depth;
        const equivalentDensity = mass / volume;

        // Physically reasonable density range:
        // - Aerogel/ultra-light foam: ~0.001 kg/m³
        // - Normal foam: ~0.01-0.1 kg/m³
        // - Plastic: ~1000 kg/m³
        // - Metal: ~8000 kg/m³
        // If density < 0.0001 kg/m³ (100 times lighter than aerogel), parameters are unreasonable
        const minDensity = 0.0001; // 0.1 g/m³
        if (equivalentDensity < minDensity) {
            return null; // Don't display inertia box
        }

        // Set minimum size to avoid box too small to see
        const minSize = 0.01;
        ...
        return {
            ...
            rotation: rotation // Principal inertia axis rotation (quaternion)
        };
    }

    /**
     * Create inertia box geometry (like Gazebo)
     */
    static createInertiaBoxGeometry(width, height, depth) {
        return new THREE.BoxGeometry(width, height, depth);
    }

    /**
     * Compute principal inertia axes of inertia matrix
     * Returns ellipsoid radius and rotation
     * @deprecated Use computeInertiaBox instead
     */
    static computeInertiaEllipsoid(inertial) {
        const Ixx = inertial.ixx || 0;
        const Iyy = inertial.iyy || 0;
        const Izz = inertial.izz || 0;
        const Ixy = inertial.ixy || 0;
        ...
        const eigenvalues = this.computeEigenvalues3x3(matrix);

        // Inertia ellipsoid radius proportional to square root of moment of inertia
        // Use scaling factor to make ellipsoid size appropriate
        const mass = inertial.mass || 1;
        const scale = 0.1 / Math.sqrt(mass); // Adjust size based on mass

        const radii = [
            Math.sqrt(Math.max(eigenvalues[0], 0)) * scale,
            Math.sqrt(Math.max(eigenvalues[1], 0)) * scale,
            Math.sqrt(Math.max(eigenvalues[2], 0)) * scale
        ];

        // If inertia is very small, use minimum size
        radii.forEach((r, i) => {
            if (r < 0.01) radii[i] = 0.01;
        });

        return {
            radii: radii,
            // Simplified rotation (can be improved to accurate eigenvectors later)
            rotation: new THREE.Euler(0, 0, 0)
        };
    }

    /**
     * Create inertia ellipsoid geometry
     * @deprecated Use createInertiaBoxGeometry instead
     */
    static createInertiaEllipsoidGeometry(radii) {
     ...
     */
    static rpyToEuler(rpy) {
        if (!rpy || rpy.length !== 3) {
            return new THREE.Euler(0, 0, 0);
        }
        // URDF uses roll-pitch-yaw order
        return new THREE.Euler(rpy[0], rpy[1], rpy[2], 'XYZ');
    }

    /**
     * Convert URDF xyz to Three.js Vector3
     */
    static xyzToVector3(xyz) {
        if (!xyz || xyz.length !== 3) {
